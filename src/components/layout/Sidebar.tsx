...
const AdminLinks = [
    ...
    { name: 'Reviews', path: '/admin/reviews', icon: 'bi-star-half' },
];
...
const StaffLinks = [
    ...
    { name: 'Appointments', path: '/staff/appointments', icon: 'bi-calendar-check' },
    { name: 'Part Requests', path: '/staff/part-requests', icon: 'bi-cart-plus' },
    { name: 'Reviews', path: '/staff/reviews', icon: 'bi-star-half' },
];
...
export const Sidebar: React.FC = () => {
    ...
    const links =
        role?.toLowerCase() === 'admin' ? AdminLinks :
            role?.toLowerCase() === 'staff' ? StaffLinks :
                CustomerLinks;

    return (
        <div
            className="d-flex flex-column border-end app-sidebar"
            style={{ width: '250px', minWidth: '250px', height: '100%' }}
        >
            ...
            <div className="d-flex flex-column flex-grow-1 p-3 overflow-auto">
                {links.map(link => (
                    <NavLink
                        key={link.path}
                        to={link.path}
