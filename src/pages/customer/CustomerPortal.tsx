...
import { Card, Row, Col, Button, Modal, Form, Spinner, Badge, ProgressBar } from 'react-bootstrap';
...
export const CustomerPortal: React.FC = () => {
    ...
    const isLoyal = totalSpent >= loyaltyThreshold;
    const progressPercent = Math.min((totalSpent / loyaltyThreshold) * 100, 100);
    const amountNeeded = Math.max(loyaltyThreshold - totalSpent, 0);
        ...
        <div className="animate-fade-in customer-portal">
            ...
            <Card className="mb-4 text-white border-0" style={{ background: 'linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)' }}>
                <Card.Body className="p-4 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-4">
                    ...
                    <div className="d-flex align-items-center gap-3 p-3 rounded" style={{ backgroundColor: 'rgba(0,0,0,0.2)', minWidth: '300px' }}>
                        {isLoyal ? (
                            <>
                                <i className="bi bi-star-fill text-warning fs-2"></i>
                                <div>
                                    <div className="fw-bold fs-5 text-warning">Loyalty Member</div>
                            ...
                            <div className="w-100">
                                <div className="d-flex justify-content-between mb-1">
                                    <span className="fw-semibold">Standard Member</span>
                                    <span className="small fw-bold">Rs. {totalSpent} / {loyaltyThreshold}</span>
                                </div>
                                <ProgressBar variant="warning" now={progressPercent} style={{ height: '8px' }} />
                                <div className="small text-white text-opacity-75 mt-1">
                                    Spend Rs. {amountNeeded} more to unlock a 10% discount!
                                </div>
                            </div>
                        )}
                    </div>
                                        ...
                                        <div key={v.vehicleId} className="p-3 rounded border border-secondary" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
                                            <div className="d-flex justify-content-between align-items-start">
                                                ...
                                                <div className="d-flex gap-1">
                                                    <Button variant="outline-primary" size="sm" onClick={() => openEditVehicle(v)}>
