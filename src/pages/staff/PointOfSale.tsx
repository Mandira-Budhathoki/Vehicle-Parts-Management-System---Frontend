...
export const PointOfSale: React.FC = () => {
    ...
    const updateQuantity = (partId: number, delta: number) =>
        setCart(cart.map(c => {
            if (c.partId !== partId) return c;
            const newQ = c.quantity + delta;
            if (newQ <= 0 || newQ > c.stock) return c;
    ...
    return (
        ...
        >
            <div className="d-flex align-items-center mb-4 gap-3">
                ...
                <h2 className="mb-0 text-dark fw-bold" style={{ letterSpacing: '0.5px' }}>
                    Point of Sale
                </h2>
            </div>
                                ...
                                <div className="d-flex flex-column gap-3">
                                    {parts.filter(p => p.stockQuantity > 0).map(part => (
                                        ...
                                        >
                                            ...
                                            <Button variant="primary" size="sm" onClick={() => addToCart(part)}>
                                                Add
                                            </Button>
                                        </div>
                                    ))}
                        ...
                        <>
                                ...
                                <tbody>
                                    {createdInvoice.salesItems?.map(item => (
                                        <tr key={item.salesItemId}>
                                            ...
                                            <td className="text-end">Rs. {item.price.toLocaleString()}</td>
                                            <td className="text-end">Rs. {item.subtotal.toLocaleString()}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
                            ...
                            <div className="p-3 rounded" style={{ background: '#f0f0ff', border: '1px solid #c7c9f9' }}>
                                ...
                                {emailResult && (
                                    ...
                                    >
                                        <i className={`bi ${emailResult.ok ? 'bi-check-circle' : 'bi-exclamation-triangle'} me-2`}></i>
                                        {emailResult.msg}
                                    </Alert>
                                )}
                            </div>
